...
def save_media(message, media_folder):
    if message.media:
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if total_size:
                    with tqdm(total=total_size, unit='B', unit_scale=True, desc=f"Baixando {file_name}") as pbar:
                        temp_path = message.download_media(file=temp_path, progress_callback=lambda d, t: pbar.update(d - pbar.n if t else 0))
                else:
            ...
            print(f"Erro ao baixar e comprimir mídia: {e}")
            return None, None
    return None, None
...
for chat in chats:
    ...
    with TelegramClient('session_name', api_id, api_hash) as client:
        all_messages = list(client.iter_messages(chat, offset_date=datetime.date.today()-datetime.timedelta(days=2), reverse=True))
            ...
            for message in all_messages:
                ...
                if message.reply_to:
                    try:
                        ...
                        reply_to_message = client.get_messages(chat, ids=reply_to_message_id)
                    except Exception as e:
                ...
                data = {
                    ...
                    "reply_to": {
                        ...
                        "reply_to_message_text": reply_to_message.text if reply_to_message else None
                    },
                    "flags": {
                        "out": message.out,
                    ...
                    "media": {
                        ...
                        "photo": {
                            "id": message.media.photo.id if message.media and hasattr(message.media, "photo") else None,
                            "access_hash": message.media.photo.access_hash if message.media and hasattr(message.media, "photo") else None,
                            "file_reference": None,
                            "date": message.media.photo.date.isoformat() if message.media and hasattr(message.media, "photo") else None,
                            "sizes": None,
                            "dc_id": None,
                            "has_stickers": None,
                            "video_sizes": None
                    ...
                    "reactions": [
                        {"reaction": reaction.reaction.emoticon, "count": reaction.count}
                ...
                with open(output_file, 'w', encoding='utf-8') as json_file:
                    json.dump(processed_messages, json_file, ensure_ascii=False, indent=4)
